...
class _HomeScreenViewState extends State<OwnerHomeView> {
  ...
  void fetchUserName() async {
    try {
      ...
      if (currentUser != null) {
        DocumentSnapshot userDoc = await FirebaseFirestore.instance.collection('owners').doc(currentUser.uid).get();

        if (userDoc.exists) {
          setState(() {
            userName = userDoc['name'] ?? 'Guest';
            gender = userDoc['gender'] ?? 'Male';
            profilePictureUrl = (userDoc.data() as Map<String, dynamic>).containsKey('profile_picture_base64')
                      ...
                      child: Skeletonizer(
                        ...
                        child: SizedBox(
                          ...
                          child: ListView.separated(
                            ...
                            itemCount: allBanquets.length > 3 ? 3 : allBanquets.length,
                            separatorBuilder: (context, index) => const SizedBox(width: 10),
                            itemBuilder: (context, index) {
                              final banquet = allBanquets[index].data() as Map<String, dynamic>;
          ...
          Column(
            ...
            children: [
              const Center(
                child: Text(
                  "Welcome Back! ",
                  style: TextStyle(fontSize: 22, fontWeight: FontWeight.w500, color: MyColors.white100),
                ),
              ),
              Center(
                child: Text(
                  userName,
                  style: const TextStyle(fontSize: 26, fontWeight: FontWeight.w500, color: MyColors.textSecondary),
                ...
                child: GridView(
                  ...
                  children: [
                    ...
                    _buildActionCard(
                      ...
                      color: Colors.orange,
                      onTap: () {
                        Get.toNamed(AppRoutes.addBanquetScreen, arguments: {'banquet': null});
                      },
                    ),
                    _buildActionCard(
                      context,
